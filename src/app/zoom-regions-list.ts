  ...
  template: `
    <div class="p-5 rounded-2xl bg-neutral-900 border border-white/5 flex flex-col gap-4">
      <div class="border-b border-white/10 pb-3 flex flex-col gap-3">
        ...
        <button type="button" (click)="addZoomRegion.emit()"
          ...
          <mat-icon style="font-size: 14px; width: 14px; height: 14px; display: flex; align-items: center; justify-content: center;">add</mat-icon>
          <span>{{ lang() === 'vi' ? 'Thêm thu phóng' : 'Add Zoom Region' }}</span>
        </button>
      </div>

      @if (zoomRegions().length === 0) {
        <div class="py-6 px-4 text-center text-xs text-neutral-500 border border-dashed border-white/5 rounded-xl">
          {{ lang() === 'vi' ? 'Chưa cấu hình thu phóng. Hãy phát video đến đoạn cần phóng to và bấm "Thêm Thu Phóng" để phóng 2x vào phân vùng mong muốn.' 
            ...
            <div class="flex flex-col gap-2 shrink-0">
              <div class="flex items-center justify-between p-3 rounded-xl border transition-all text-xs select-none"
                ...
                <div class="flex items-center gap-2 shrink-0">
                  <button type="button" (click)="deleteZoomRegion.emit(zoom.id)" 
              ...
              @if (activeZoomId() === zoom.id) {
                <div class="p-4 rounded-xl bg-neutral-950 border border-emerald-500/20 flex flex-col gap-4 animate-fade-in mb-2 mx-0.5">
                  <div class="text-xs font-semibold text-neutral-300 flex items-center justify-between">
                    <span class="flex items-center gap-1.5 font-medium">
                      <mat-icon class="text-emerald-400" style="font-size: 14px; width: 14px; height: 14px; display: flex; align-items: center; justify-content: center;">center_focus_strong</mat-icon>
                      {{ lang() === 'vi' ? 'Cấu hình vùng tiêu điểm' : 'Focus Area Configuration' }}
                    </span>
                  </div>
                  ...
                  <div class="flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                      <div class="text-[10px] font-medium text-neutral-400 flex justify-between">
                        <span>{{ lang() === 'vi' ? 'Thời điểm bắt đầu' : 'Start Time' }}</span>
                        <span class="font-mono text-neutral-300">{{ zoom.startTime.toFixed(2) }}s / {{ formatTimeShort(videoDuration()) }}</span>
                      </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                      <div class="text-[10px] font-medium text-neutral-400 flex justify-between">
                        <span>{{ lang() === 'vi' ? 'Thời lượng hiển thị' : 'Duration' }}</span>
                        <span class="font-mono text-neutral-300">{{ zoom.duration.toFixed(1) }}s / {{ formatTimeShort(videoDuration()) }}</span>
                      </div>
                      <div class="flex items-center gap-2">
                        <input type="range" 
                  ...
                  <div class="flex flex-col gap-2 border-t border-white/5 pt-3">
                    <span class="text-[10px] font-semibold text-neutral-400 uppercase tracking-wide">
                      {{ lang() === 'vi' ? 'Nhấp chọn 1 trong 4 phân vùng để phóng to (2x)' : 'Click 1 of 4 quadrants to focus & zoom (2x)' }}
                    </span>
